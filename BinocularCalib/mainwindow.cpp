...
Size chessboardSize = Size(41, 41); // 棋盘上每个棋盘格的大小30mm
Size imageSize; // 图像尺寸
Rect validRoi[2];
...
void SaveParameters()
{
    cv::FileStorage fs("calibParams.yml", cv::FileStorage::WRITE);
    if (fs.isOpened())
    {
...
void MainWindow::handleTimeout()
{
    if(camera.isOpened())
    {
        ...
        if(m_bSaveImage)
        {
            ...
            QString strFrmNum = "保存了" + QString::number(cnt) + "帧";

            ui->lblFrmNum->setText(strFrmNum);
